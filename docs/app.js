...
async function generateSlotNow(slot, statusEl) {
  ...
  // Recompute "today" fresh right here rather than trusting whatever
  // currentDateStr happens to hold - this button is only ever shown for
  // today's slots (isViewingHistorical() hides it otherwise), so the
  // date sent to the backend should reflect the actual moment the
  // button was pressed, not a value cached from the last render/poll.
  const result = await triggerGenerateSlot(todayStrIST(), slot.index);
  if (!result.ok) {
...
async function postSlotNow(slot, lang, statusEl) {
  ...
  // Same reasoning as generateSlotNow: recompute "today" fresh at the
  // moment of the click rather than trusting a possibly-stale
  // currentDateStr.
  const result = await triggerPostNow(todayStrIST(), slot.index, lang);
  if (!result.ok) {
...
function openModal(slot) {
  ...
  manualBtn.onclick = async () => {
    ...
    // Same reasoning as generateSlotNow/postSlotNow: use freshly-computed
    // "today" rather than a possibly-stale currentDateStr.
    const ok = await markSlotManual(todayStrIST(), slot.index, takeoverLang);
    if (ok) {
...
function renderModalLang(lang) {
  ...
  const hasStories = (slot.stories || []).length > 0;
  if (!imageUrls.length && lang === "hi" && hasStories) {
    ...
    const p = document.createElement("p");
    p.className = "field-hint";
    p.textContent = "No content built for this slot yet.";
    scroller.appendChild(p);
    shareBtn.hidden = true;
  } else {
...
async function saveReviewSelection() {
  ...
  // Same reasoning as generateSlotNow/postSlotNow: use freshly-computed
  // "today" rather than a possibly-stale currentDateStr.
  const result = await saveSlotSelection(todayStrIST(), reviewSlot.index, reviewSelected);
  if (result.ok) {
